...
{
  for (auto i : pendingUnit){
    if (!i.first->exists())
      pendingUnit.erase(i.first);
    else{
      if (i.second.macroAct.isUnit()){
        if (i.first->getType() == i.second.macroAct.getUnitType())
          pendingUnit.erase(i.first);
        else if (i.second.macroAct.isAddon() && i.first->getAddon() != nullptr)
          pendingUnit.erase(i.first);
        else if (i.first->getTrainingQueue().size() > 0 && i.first->getTrainingQueue().front() == i.second.macroAct.getUnitType())
          pendingUnit.erase(i.first);

      }
      else if (i.second.macroAct.isUpgrade()){
        if (i.first->getUpgrade() == i.second.macroAct.getUpgradeType())
          pendingUnit.erase(i.first);
      }
      else if (i.second.macroAct.isTech())
...
void ProductionManager::create(BWAPI::Unit producer, const BuildOrderItem & item)
{
  ...
  if (act.isAddon())
  {
    ...
    pendingUnit.emplace(producer, item);
  }
...
{
  int mins = 0;
  for (auto e : pendingUnit){
    mins += e.second.macroAct.mineralPrice();
  }
  return BWAPI::Broodwar->self()->minerals() - BuildingManager::Instance().getReservedMinerals() - mins;
}
...
int ProductionManager::getFreeGas() const
{
  int gas = 0;
  for (auto e : pendingUnit){
    gas += e.second.macroAct.gasPrice();
  }
  return BWAPI::Broodwar->self()->gas() - BuildingManager::Instance().getReservedGas() - gas;
}
